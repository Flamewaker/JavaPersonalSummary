# Flink 
1. Apache Flink is an open source platform for distributed stream and batch data processing.
2. 两种数据集合
(1) unbounded数据集合：无边界数据集合=>实时计算 =>Flink DataStream API。
(2) bounded数据集合：有边界的数据集合=>离线计算=>Flink DataSet API.
3. Flink的生态圈
(1) 平台层: local模式(JAVA, Scala)，cluster模式(standalone(自己管理), Yarn), Cloud模式(docker, k8s, aws)。
(2) 核心层：Flink Runtime 执行引擎。
(3) API & Library: API(DataSet, DataStream),Library(CEP 复杂事件处理, Table API & SQL, MLlib, Gelly)。
4. Flink的体系架构
(1) 服务器端(主从架构)：JobManager主节点, TaskManager从节点(Task Slot进程(task, task), Task Slot进程(task))(并行度不要超过cpu核数)。
(2) 客户端：Flink Program(创建核心的对象)=>(DataSet API: ExecutionEnvironment, DataStreamAPI: StreamExecutionEnvironment)(类似SparkContext)
5. 编程结构
```
...
[DataSet Transformation官网链接](https://flink.sojb.cn/dev/batch/dataset_transformations.html#filter)
#### 3.1 Map
采用一个数据元并生成一个数据元。(一对一)
```
...
#### 3.5 Reduce
通过将两个数据元重复组合成一个数据元，将一组数据元组合成一个数据元。Reduce可以应用于完整数据集或分组数据集。(多变一)
```
...
#### 3.7 Aggregate
将一组值聚合为单个值。聚合函数可以被认为是内置的reduce函数。聚合可以应用于完整数据集或分组数据集。(仔细注意这个地方，其简写方式，对一个分组的数据可以将整个分组数据进行处理，部分方法可以用reduce进行实现)
...
#### 3.14 Rebalance	
均匀地Rebalance 数据集的并行分区以消除数据偏差。只有类似Map的转换可能会遵循Rebalance转换。
